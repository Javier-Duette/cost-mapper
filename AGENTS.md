# AGENTS.md — Punto de entrada canonico para agentes de IA

> **Este archivo va en la raíz del repositorio de código.** Todo agente de IA (Codex, Claude Code, Copilot, Cursor) debe leerlo antes de proponer cualquier cambio. Es el briefing operativo canonico del proyecto.
>
> `CLAUDE.md` queda solo como archivo de compatibilidad para herramientas que lo buscan automaticamente. Si hay contradiccion entre `AGENTS.md` y `CLAUDE.md`, prevalece `AGENTS.md`.
...
2. **Este archivo (`AGENTS.md`)** — reglas del proyecto, convenciones de código, módulos y protocolo de git.
...
| [ADR-011](docs/adrs/ADR-011.md) | Nodos NBR vs ítems TCPO | Explica `is_work_item` y por qué el catálogo mezcla árbol NBR e ítems presupuestables |
| [ADR-012](docs/adrs/ADR-012.md) | Extracción TCPO con LLM Vision | Explica el pipeline real de extracción desde PDF rasterizado |
| [ADR-013](docs/adrs/ADR-013.md) | Fuente canónica para agentes IA | Declara `AGENTS.md` como entrada oficial y `CLAUDE.md` como compatibilidad |
...
```
cost-mapper/
├── AGENTS.md              ← estás aquí; fuente canónica para agentes
├── CLAUDE.md              ← puente de compatibilidad; no agregar reglas nuevas
...
| `docs/design-system/` | — | Código estático generado por Codex Design. Es **sólo lectura**. NO editar manualmente. Úsalo como referencia visual. |

---

## Flujo de Trabajo: Sistema de Diseño (Codex Design)

Cuando el diseñador (o Codex Design) provea una actualización del diseño, entregará un archivo `.zip` en la raíz del repositorio. El protocolo de actualización es estricto:
...
4. **Documentación de Implementación (Feedback Loop):** Después de implementar la interfaz en el frontend real, el agente de IA **DEBE** documentar detalladamente qué se ejecutó en la realidad frente a lo que pedía el mockup (ej. estructura final de componentes de React, limitaciones técnicas, decisiones de simplificación). Esto permitirá que en la próxima iteración, Codex Design tenga contexto de cómo se llevó su mockup a la vida real y diseñe sobre la arquitectura real y no sobre ilusiones pasadas.
...
| Las instrucciones del repo | `AGENTS.md` |
